...
- 公开网站固定页头右侧、`INDEX` 左侧显示“管理”入口，链接至同域名 `admin.html`；不再把入口放在需要继续下滑才能看到的结尾底栏。
- 入口只负责导航，不改变安全边界。访问后台仍必须通过现有 CloudBase 管理员登录，网站不保存或暴露密码。
- 桌面和手机均固定可见；“管理”与 `INDEX` 使用细分隔线区分，页头中间栏目状态、底部品牌栏和 `BACK TO TOP` 行为不变。当前公共 CSS 缓存版本为 `20260724-header-admin-2`。
