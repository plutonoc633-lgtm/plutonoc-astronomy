...
- B站播放能力的公共 CSS / JS 与后台 CSS / JS 缓存版本为 `20260904-bilibili-1`。修改该功能后需同步部署 Pages、`plutonoc-content-publisher` 云函数和 CloudBase 管理后台。
- 东京旅行 Vlog 已于 2026-09-04 使用 `BV1sv4Q6sECW` 加入动态影像并置顶，站内只保存两档 WebP 封面，播放由 B 站嵌入播放器承担；视频内容版本为 `20260904-tokyo-vlog-1`，12.1GB 本地原片未进入 Git。
