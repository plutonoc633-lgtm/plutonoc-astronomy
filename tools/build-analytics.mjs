...
for (const role of ['collect', 'query', 'cleanup']) {
  ...
  if (role === 'cleanup') {
    const secretPath = path.join(directory, 'cleanup-secret.json');
    if (!fs.existsSync(secretPath)) fs.writeFileSync(secretPath, JSON.stringify(crypto.randomBytes(32).toString('hex')));
